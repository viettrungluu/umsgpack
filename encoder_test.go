...
func TestTimestampExtensionMarshalTransformer(t *testing.T) {
	if obj, err := TimestampExtensionMarshalTransformer("hi"); err != nil || obj != "hi" {
		t.Errorf("Unexpected result: %#v, %v", obj, err)
	}
	...
	testCases := []struct {
		...
		// timestamp 32
		{tm: time.Unix(0, 0), data: []byte{0x00, 0x00, 0x00, 0x00}},
		{tm: time.Unix(math.MaxUint32, 0), data: []byte{0xff, 0xff, 0xff, 0xff}},
		// timestamp 64
		{tm: time.Unix(math.MaxUint32+1, 0), data: []byte{0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00}},
		{tm: time.Unix(1<<34-1, 0), data: []byte{0x00, 0x00, 0x00, 0x03, 0xff, 0xff, 0xff, 0xff}},
		{tm: time.Unix(0, 1), data: []byte{0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00}},
		{tm: time.Unix(0, 999999999), data: []byte{0xee, 0x6b, 0x27, 0xfc, 0x00, 0x00, 0x00, 0x00}},
		{tm: time.Unix(1<<34-1, 999999999), data: []byte{0xee, 0x6b, 0x27, 0xff, 0xff, 0xff, 0xff, 0xff}},
		// timestamp 96
		{tm: time.Unix(1<<34, 0), data: []byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00}},
		{tm: time.Unix(math.MaxInt64, 0), data: []byte{0x00, 0x00, 0x00, 0x00, 0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}},
		{tm: time.Unix(1<<34, 1), data: []byte{0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00}},
		{tm: time.Unix(1<<34, 999999999), data: []byte{0x3b, 0x9a, 0xc9, 0xff, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00}},
		{tm: time.Unix(-1, 0), data: []byte{0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}},
		{tm: time.Unix(math.MinInt64, 0), data: []byte{0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}},
		{tm: time.Unix(-1, 1), data: []byte{0x00, 0x00, 0x00, 0x01, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}},
		{tm: time.Unix(-1, 999999999), data: []byte{0x3b, 0x9a, 0xc9, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}},
		{tm: time.Unix(math.MinInt64, 1), data: []byte{0x00, 0x00, 0x00, 0x01, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}},
		{tm: time.Unix(math.MinInt64, 999999999), data: []byte{0x3b, 0x9a, 0xc9, 0xff, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}},
	}
	for i, tc := range testCases {
		obj, err := TimestampExtensionMarshalTransformer(tc.tm)
		if err != nil {
		...
		if !ok || ext == nil || ext.ExtensionType != -1 || bytes.Compare(ext.Data, tc.data) != 0 {
			t.Errorf("%v: Unexpected result: %#v", i, obj)
			continue
		}
	}
}
